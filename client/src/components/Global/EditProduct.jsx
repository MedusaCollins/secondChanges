...
const EditProduct = ({ isModalOpen,product }) => {
  ...
  async function handleReviewSubmit() {
    try {
      ...
      const response = await axios.post(`${process.env.REACT_APP_DB}/editProduct`, formData);
                  ...
                  return (
                    <div key={key} className='w-full h-16 flex items-center justify-between rounded p-3 bg-white dark:bg-[#212529]'>
                      <div className="flex flex-row items-center gap-2">
                        ...
                        <span className="truncate w-44 dark:text-white">{file}</span>
                      </div>
